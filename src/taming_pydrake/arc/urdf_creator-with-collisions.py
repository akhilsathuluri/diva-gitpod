...
# a1, lua, a23_distr, lfa, a45_distr, rho, s2, s4, K, taumax, m_m, N
# sample_dv = [0.332,0.3655,0.5,0.335,0.5,1000.5,0.07,0.0396, 5.5, 22.775, 0.8, 6]
# Nominal check
sample_dv = [0.332,0.3655,0.5,0.335,0.5,1000,0.07,0.0396, 5.5, 17.5, 0.65, 8]
# sample check-1: Good design as expected: tau_max: 14.423763366255708
# sample_dv = [0.3628,0.3655,0.139,0.335,0.5,1000,0.07,0.0396, 5.5, 17.5, 0.65, 8]
# sample check-2: is also a good design but is seen as a bad design for some reason
# sample_dv = [0.30366,0.3655,0.7335,0.335,0.5,1000,0.07,0.0396, 5.5, 17.5, 0.65, 8]
# sample_dv = [0.302933,0.3655,0.0902,0.335,0.5,1000,0.07,0.0396, 5.5, 17.5, 0.65, 8]

# Map from sampled dv to actual urdf params
# a1, lua, a23_distr, lfa, a45_distr, rho, s2, s4, K, taumax, m_m, N --->
# a1, a2, a3, a4, a5, rho, s2, s4, K, taumax, m_m, N
def dv_sampled2dv_urdf(dv):
    return [dv[0], dv[1]*dv[2], dv[1]*(1-dv[2]), dv[3]*dv[4], dv[3]*(1-dv[4])]+ dv[5:]
...
def urdf_creator(sampled_dv):
    # New dvs are a1, a2, a3, a4, a5, rho, s2, s4, K, taumax, m_m, N
    ...
    my_robot(
        ...
        Joint("joint-joint-2",
              Parent("link2"),
              Child("joint-link-2"),
              Origin(xyz=list2space([0,0,a[1]]), rpy="1.5707 0 0"),
              Axis(xyz="1 0 0"),
              type="fixed"),
        ...
        Joint("joint-joint-3",
              Parent("link3"),
              Child("joint-link-3"),
              Origin(xyz=list2space([0,0,a[2]]), rpy="1.5707 0 0"),
              Axis(xyz="1 0 0"),
              type="fixed"),
        ...
        Joint("joint-joint-4",
              Parent("link4"),
              Child("joint-link-4"),
              Origin(xyz=list2space([0,0,a[3]]), rpy="1.5707 0 0"),
              Axis(xyz="1 0 0"),
              type="fixed"),
        ...
        Joint("joint-joint-5",
              Parent("link5"),
              Child("joint-link-5"),
              Origin(xyz=list2space([0,0,a[4]]), rpy="1.5707 0 0"),
              Axis(xyz="1 0 0"),
              type="fixed"),
        ...
        #       Parent("link6"),
        #       Child("joint-link-6"),
        #       Origin(xyz=list2space([0,0,templ[0]]), rpy="1.5707 0 0"),
        #       Axis(xyz="1 0 0"),
        #       type="fixed"),
        #
        # # Joint-link-6
