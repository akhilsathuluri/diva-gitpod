...
# var_list = np.random.rand(15,2).tolist()
# results = Parallel(n_jobs=-1,backend='multiprocessing')(delayed(compute_QoI)(pre_val, dv_names, var_list[i], case) for i in range(len(var_list)))

prob = pg.problem(Baseline_Problem(pre_val, dv_names, dv_bounds, case))
algo = pg.algorithm(pg.cmaes(gen = 10, sigma0=0.3))
archi = pg.archipelago(n=1, algo = algo, prob=prob, pop_size=70, b = pg.default_bfe())
archi.evolve()
archi.wait()
